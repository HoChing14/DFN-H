...
from tough_script import main_script, write_string, run_tough


def read_obs(filename, short='0'):
    t_obs, c_obs, sh_t_obs, sh_c_obs = [], [], [], []
    ...
    return t_obs, c_obs, sh_t_obs, sh_c_obs
...
#ax2, ax3 = fig.add_subplot(121), fig.add_subplot(122)
...
with open('source_sink.txt') as sst:
    for jis in sst.readlines():
        ...
        ss_g.append(jcs)
c_t, c_c, c11, c12 = read_obs('obs_con.txt')
T_t, T_c, sh_T_t, sh_T_c = read_obs('obs_T.txt', short_time)
p_t, p_c, p11, p12 = read_obs('obs_p.txt')
...
w_c, w_s, w_st = parse_con_p_meshes(ele, con, top_T, bot_T, h_top, h_bot, T_loss, inj_T, ref_d=3500.0)
...
p_ini, T_ini = [3.008219029639E1, 3.002668877677E1], [188.387884520727, 195.991910950624]
...
ccsc, ccsp, interp_p, tc = compliklihood(ttem, ptem, p_t, p_c)
for ae, af in zip(T_t, interp_T):
    w_tem += f'{ae},{af}\n'
for ag, ah in zip(p_t, interp_p):
    w_tep += f'{ag},{ah}\n'
