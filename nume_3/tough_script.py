...
def uge2tough2(cell, connection, ss, aperture, anay):
    ...
    for t in range(len(cell)):
        ...
        cell_volume, center, belonging = f'{data_convert(cell[t][4] * aperture)}', cell[t][1:4], []
    ...
    for i in range(len(connection)):  # read and calculate connection data
        ...
        area = connection[i][5] * aperture  # contact area
...
def run_tough(dirt, tss, flo_inp, chem_inp, t_unit, inip, iniT, n_mo=1, c=True):
    ...
    for i, j, k in zip(flo_inp, chem_inp, tss):
        ...
        if not t_T:
            break
        ori_io = [f'{dirt}/kdd_tim.dat', f'{dirt}/SAVE', f'{dirt}/FOFT', f'{dirt}/GOFT', f'{dirt}/savechem', f'{dirt}/kdd_conc.dat']  # restart
        des_io = [f'{dirt}/kdd_tim_{stat}.dat', f'{dirt}/INCON', f'{dirt}/FOFT_{stat}', f'{dirt}/GOFT_{stat}', f'{dirt}/inchem', f'{dirt}/kdd_conc_{stat}.dat']
        shutil.copy(f'{dirt}/INCON', f'{dirt}/INCON_{stat}')
